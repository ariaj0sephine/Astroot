...
class _VisibleTonightScreenState extends State<VisibleTonightScreen> {
  final TextEditingController _latController = TextEditingController(text: '11.0510');
  final TextEditingController _lonController = TextEditingController(text: '76.0711');
  ...
  // Hardcoded bright stars (easy and fast for your phone)
  ...
  // Hardcoded bright planets (good enough for demo)
    ...
    try {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          _cityName = data['address']['city'] ?? data['address']['town'] ?? 'Unknown';
        }
      } catch (_) {
  ...
  // ====================== SIMPLE MATH TO FIND WHAT IS ABOVE HORIZON ======================
  double _calculateJD(DateTime dt) {
    ...
    int b = 2 - a + (a / 4).floor();
    return (365.25 * (year + 4716)).floor() + (30.6001 * (month + 1)).floor() + day + b - 1524.5 + ut / 24.0;
  }
  ...
  Future<void> _fetchData() async {
    ...
    final String sunUrl = 'https://api.sunrise-sunset.org/json?lat=$lat&lng=$lon&date=$apiDate&formatted=0';
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'OK') {
          ...
          if (sunriseUtc != 'N/A') {
            ...
            sunriseLocal = DateFormat('hh:mm a').format(istTime);
          }
          if (sunsetUtc != 'N/A') {
            DateTime utcTime = DateTime.parse(sunsetUtc);
            DateTime istTime = utcTime.add(const Duration(hours: 5, minutes: 30));
            sunsetLocal = DateFormat('hh:mm a').format(istTime);
          }
    ...
    for (var planet in _brightPlanets) {
      ...
      if (alt > 20 && (planet['mag'] as double) < 2) {
        _planetsData.add({'name': planet['name']});
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoadingLocation || _isFetchingData
          ? const Center(child: CircularProgressIndicator(color: Color(0xFF64B5FF)))
          : Container(
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                          ...
                          children: [
                            Expanded(
                              child: ListTile(
                                ...
                                title: Text(_cityName, style: const TextStyle(color: Colors.white)),
                              ),
                            ),
                            Expanded(
                              child: Align(
                        ...
                        // ── Sunrise & Sunset card ──
                        const Text('Sunset & Sunrise', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                        const SizedBox(height: 12),
                        Container(
                          decoration: BoxDecoration(
                            gradient: const LinearGradient(colors: [Color(0xFF1E244B), Color(0xFF2D325A)]),
                          ...
                          child: Column(
                            children: [
                              ListTile(
                                ...
                                title: Text('Sunrise: ${_sunMoonData['sunrise'] ?? 'N/A'}', style: const TextStyle(color: Colors.white)),
                              ),
                              ListTile(
                                leading: const Icon(Icons.wb_twilight, color: Colors.white),
                                title: Text('Sunset: ${_sunMoonData['sunset'] ?? 'N/A'}', style: const TextStyle(color: Colors.white)),
                              ),
                            ],
                          ),
                        )
                        ...
                        // ── Moon Phase card ──
                        const Text('Moon Phase', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                        const SizedBox(height: 12),
                        Container(
                          decoration: BoxDecoration(
                            gradient: const LinearGradient(colors: [Color(0xFF1E244B), Color(0xFF2D325A)]),
                        ...
                        // ── Bright Planets card ──
                        const Text('Bright Planets Visible', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                        const SizedBox(height: 12),
                        Container(
                          decoration: BoxDecoration(
                          ...
                          child: Column(
                            children: _planetsData.isEmpty
                                ? [const Padding(padding: EdgeInsets.all(16), child: Text('No bright planets visible right now', style: TextStyle(color: Colors.white70)))]
                                : _planetsData
                                .asMap()
                                .entries
                                .map<Widget>(
                                ...
                                title: Text(entry.value['name'], style: const TextStyle(color: Colors.white)),
                                  ...
                                  .animate()
                                  .fade(duration: 500.ms, delay: Duration(milliseconds: 200 + entry.key * 80))
                        ...
                        // ── Bright Stars card ──
                        const Text('Bright Stars Visible', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                        const SizedBox(height: 12),
                        Container(
                          decoration: BoxDecoration(
                            color: const Color.fromRGBO(51, 57, 118, 1),
                          ...
                          child: Column(
                            children: _starsData.isEmpty
                                ? [const Padding(padding: EdgeInsets.all(16), child: Text('No bright stars visible right now', style: TextStyle(color: Colors.white70)))]
                                : _starsData
                                .asMap()
                                .entries
                                .map<Widget>(
                                ...
                                title: Text(entry.value['name'], style: const TextStyle(color: Colors.white)),
                                  ...
                                  .animate()
                                  .fade(duration: 500.ms, delay: Duration(milliseconds: 300 + entry.key * 80))
                                  .slideY(begin: 0.15, end: 0),
                            )
                                .toList(),
                          ),
